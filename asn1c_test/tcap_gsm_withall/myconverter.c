...
static void * data_decode_from_buffer(asn_TYPE_descriptor_t *pduType, uint8_t *i_bptr, size_t i_size);
...
int hexstr_to_bytes(unsigned char *outbytes, size_t maxout, char *instr,  size_t inlen) {
	int i, j;
	unsigned char buf;
	unsigned char *res = outbytes;
	if(!inlen || inlen %2 ) {
		return -2;
	} else {
        for(i = 0, j=0 ; i < inlen; i++){
                if(i % 2 != 0){
                        res[j++] =  hex_to_uchar(buf, instr[i]);
                }else{
                        buf = instr[i];
                }
        }
...
int convert_hexstr_to_bytes(const char *hex_str, unsigned char *byte_array, int byte_array_max)
{
...
char *strcpynospace(char *out, char *in,  size_t maxout) {
	size_t inlen = strlen(in);
	int i = 0, j = 0;
	for (i = 0, j = 0; i< inlen && j < maxout; i++) {
		char ci = in[i];
		 if (ci == ' ' || ci == '\t' || ci == '\n') {
		 	continue;
		 } else {
...
main(int ac, char *av[]) {
	...
	int num;
	int ch;
	char inputhexstr[2000] = {0};
	unsigned char inputbytes[1000] = {0};
	int inputbyteslen = 0;
	 ...
	 */
	while((ch = getopt(ac, av, "i:o:1b:cdn:p:hs:H:" JUNKOPT)) != -1)
	switch(ch) {
	case 'i':
	...
	case 's':
		...
		break;
		case 'H':
			strcpynospace(inputhexstr, optarg, sizeof(inputhexstr));
			// strncpy(inputhexstr, optarg, sizeof(inputhexstr));
		break;
#ifdef	JUNKTEST
	case 'J':
...
#define	_ASX(x)	_AXS(x)
		...
		"  -H <hexstring> use this hextsting instead of using file as input\n"
#ifdef	JUNKTEST
	...
	for(num = 0; num < number_of_iterations; num++) {
	  ...
	  if(inputhexstr[0]) {
		...
		//if ((inputbyteslen = hexstr_to_bytes(inputbytes, 1000, inputhexstr, strlen(inputhexstr)))<= 0) {
		if ((inputbyteslen = convert_hexstr_to_bytes(inputhexstr, inputbytes, 1000))<= 0) {
	  ...
	  for(ac_i = 0; (ac_i < ac) || (inputhexstr[0] && ac_i == 0); ac_i++) {
		printf("ac: %d\n", ac);
	    asn_enc_rval_t erv;
	    ...
	    for(first_pdu = 1; first_pdu || !opt_onepdu; first_pdu = 0) {
			...
			*/
			if (inputhexstr[0]) {
				fprintf(stdout, "%s\n", inputhexstr);
				structure = data_decode_from_buffer(pduType,
						inputbytes, inputbyteslen);
			} else {
