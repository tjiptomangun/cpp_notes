...
int main(int argc,	char **argv) {
	...
	unsigned char outpacked[100] = {0};
	if (argc < 2) {
		fprintf(stderr, "%s <ascii string>", argv[0]);
		exit(0);
	}
	else {
		int length_out = ascii_to_gsm7(argv[1], out, strlen(argv[1]), 100);
		pack_gsm7_bit(out, outpacked, length_out, 100);
		print_bytes(outpacked, length_out);
	}
}
