...
/*
 * this is our tlv tree structure
 * we will play it as windows explorer treeitem.
 * in the first phase we will have no recursive operation
 * yet.
 */
typedef struct tlvtreeitem
{
	 struct primclass primclass;
	 struct tlvtreeitem *next;/*sibling*/
	 struct tlvtreeitem *parent;/*pointer to parent*/
	 struct tlvtreeitem *head; /*child head*/
	 struct tlvtreeitem *tail; /*child tail*/
	...
	delete
	 ...
	 unsigned char tag[10]; /*this should be 4 bytes maximum*/
	 int taglen;
	 int valuelen;   
	 unsigned char *value;
	 
}TLVTREEITE, *PTLVTREEITEM;
#endif
