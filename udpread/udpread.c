/**
 * gcc32 -Wall -ggdb3 udpread.c -o udpread
 *
 **/
#include <stdlib.h>
#include <stdio.h>
...
{ 
   ...
   fprintf (stdout, "sigchld return\n");
}
sigset_t  SigSet;
#define MAX_PACKET_LEN 10
int usage (char *appName)
{
	fprintf (stderr, "usage %s port_number [spawn] [send] [norecv] [noexit]\n", appName);
	return 0;
}
int main(int argc, char* argv[])
{
   ...
   int  nRet;
   int  nRs;
   unsigned char bnoexit = 0; /*exit if receive sigchld */
   unsigned char bnorecv = 0; /*exit if receive sigchld */
   ...
   if(argc>2)
   {
        ...
        tmp[3]=='w'&&tmp[4]=='n'&&tmp[5]==0 )
     {
	    ...
	    {
		tmp = (char *) argv[4];
		if (tmp[0]== 'n' && tmp[1]== 'o' && tmp[2] == 'e' && 
		  tmp[3] == 'x' && tmp[4] == 'i' && tmp[5] == 't')
		{
			bnoexit  = 1;
		}
		else if (tmp[0] == 'n' && tmp[1] == 'o' && tmp[2] == 'r' &&
		  tmp[3] == 'e' && tmp[4] == 'c' && tmp[5] == 'v')
		{
			bnorecv = 1;
		}
	    }
