...
static char * __property_getvalue (PPROPERTY p)
{
	return p->value;
}

static char *__property_getname (PPROPERTY p)
{
	return p->l_item.class.name;
}
...
PPROPERTY newproperty (char *name)
{
	...
	{	
		...
		prop->getname = __property_getname;
	}
	return prop;
}

PPROPERTY newproperty_value (char *name, char *value)
{
	PPROPERTY prop = (PPROPERTY) calloc (1, sizeof (PROPERTY));
	if (prop)
	{	
