...
static int __class_writetofile (PCLASS p, FILE *fp)
{
	fprintf (fp , "%s\n", p->name);
	return 0;
}
...
{ 
	...
	pclass->writetofile = __class_writetofile; 
}
...
void __l_item_ctor (PL_ITEM pl_item, char *name)
{
	...
	pl_item->class.writetofile = __class_writetofile;
}
...
static int __list_writetofile (PLIST p, FILE *fp)
{

	PL_ITEM pl_item = NULL;
	__class_writetofile(&p->l_item.class, fp);
	pl_item = p->head;
	while (pl_item)
	{
...
/**
 * NAME		: __list_add
 * DESCRIPTION	: static function that add item to list class 
 */
...
/**
 * NAME		: __list_take
 * DESCRIPTION	: take and detach head
 */
...
/**
 * NAME		: __list_delete
 * Description	: override class delete for this structure
 */
...
{ 
	...
	plist->l_item.class.writetofile = 
		(int(*)(PCLASS, FILE *))(__list_writetofile); 
...
static int __property_writetofile (PPROPERTY p, FILE *fp)
{
	fprintf (fp, "%s = %s\n", p->l_item.class.name, p->value);
	return 0;
}
/**
...
void __property_ctor (PPROPERTY prop, char *name)
{
	...
	prop->l_item.class.writetofile = 
		(int (*) (PCLASS, FILE *))(__property_writetofile);
}
...
static int __blob_printattributes(PBLOB p, int ident)
{
	__l_item_printattributes(&p->l_item, ident);	
	print_identation (ident);
	fprintf (stdout, "blob.size: %d", p->size);
	fprintf (stdout, "\n");
	fprintf (stdout, "blob.value: %s", p->value);
	fprintf (stdout, "\n");
	return 0;
}


static int __blob_writetofile (PPROPERTY p, FILE *fp)
{
	fprintf (fp, "%s = %s\n", p->l_item.class.name, p->value);
	return 0;
}
 ...
 *		0 on success
 */
static int __blob_delete (PBLOB p)
{
	p->l_item.next = NULL;
	memset (p->value, 0, p->size);
	free (p->value);
	return __l_item_delete (&p->l_item); 
}

/**
 * NAME		: __blob_ctor 
 * DESCRIPTION	: Constructor for blob
 * INPUT	  
 *		- blob, self pointer
 *		- name, object name
 * RETURNS
 ...
 *	other	: failed
 */
int __blob_ctor (PBLOB blob, char *name, int sz, unsigned char *value)
{
	__l_item_ctor (&blob->l_item, name);
	blob->l_item.class.this = (PCLASS) blob;
	blob->l_item.class.type = CLASS_BLOB;
	strncpy (blob->l_item.class.name, name, MAX_NAME_LENGTH - 1);
	if ((blob->value = calloc (1, sz)) == NULL){
		return -1;
	} 
	blob->size = sz;
	memcpy (blob->value, value, sz); 
	blob->l_item.next = NULL;
	blob->l_item.class.printattributes = 
		(int (*) (PCLASS, int))(__blob_printattributes);
	blob->l_item.class.delete = 
		(int (*) (PCLASS))(__blob_delete);
	blob->l_item.class.writetofile = 
		(int (*) (PCLASS, FILE *))(__blob_writetofile); 
	return 0;
}

PBLOB newblob (char *name, int sz, unsigned char *blob){
	PBLOB pdest = (PBLOB) calloc (1, sizeof (BLOB));
	if (pdest){
		if (__blob_ctor (pdest, name, sz, blob)){
			free (pdest);
			return NULL;
		}
	}
...
PMINIPARSER newminiparser (FILE *in, char *tokenlist, int (*f)(PMINIPARSER, void  *))
{
...
PFILEPARSER newfileparser (FILE *in,  int (*f)(PFILEPARSER, PLIST *))
{
	PFILEPARSER p = (PFILEPARSER) calloc (1, sizeof (FILEPARSER));
	p->input = in; 
	p->parse = f;	
	return p;
}
...
/*
static void  __treeitem_listadd (struct tree_item *root, PL_ITEM added)
{
	root->list.add(&root->list, added); 
}

static PL_ITEM __treeitem_listgetname (struct tree_item *root, char *name)
{
	return root->list.getname (&root->list, name); 
}

static PL_ITEM __treeitem_listgetfirstchild (struct tree_item *root)
{
	return root->list.getfirstchild (&root->list); 
}

static PL_ITEM __treeitem_listgetnextchild (struct tree_item *root)
{ 
	return root->list.getnextchild (&root->list); 
}

static PL_ITEM __treeitem_listdetach(struct tree_item *root, PL_ITEM detached)
{
	return root->list.detach (&root->list, detached); 
}
*/
/**
...
void __treeitem_ctor (PTREE_ITEM  new, PTREE_ITEM parent, char *name)
{
	...
	new->list.l_item.class.writetofile = 
		(int (*)(PCLASS, FILE *))(__list_writetofile);
...
int  __circularitem_printattributes (PCIRCULARITEM citem, int ident)
{
	...
	print_identation (ident);
	fprintf (stdout, "prev : %09x\n", (unsigned int) citem->prev);
	print_identation (ident);
	fprintf (stdout, "next : %09x\n", (unsigned int) citem->next); 
	return 0;
...
/**
 * Name		: __circularlist_writetofile
 * Description	: write to file
 */

static int __circularlist_writetofile (PCIRCULARLIST clist, FILE *fp)
{
	PCIRCULARITEM citem;
	int k  = clist->count;
	__class_writetofile (&clist->class, fp);
	citem = clist->current;
	while (k)
	{
		citem->class.writetofile (&citem->class, fp);
		citem = citem->next;
		k --;
	}
	return 0;
...
void __circularitem_ctor (PCIRCULARITEM pciri, char *name, void *data, int size)
{
	...
	pciri->datasize = size;
	pciri->data = data;
	pciri->class.writetofile = __class_writetofile;
...
void __circularlist_ctor (PCIRCULARLIST pcirlist, char *name)
{
	...
	pcirlist->class.writetofile = 
		(int (*) (PCLASS, FILE *))(__circularlist_writetofile);
