...
/* CC -ggdb3 -Wall XLAdmAgent.c -o XLAdmAgent -lpthread  */
...
pid_t spawn (const char *path, int fd_count, const int fd_map[], char *argv[])
{
	pid_t pid_spawn = -1;
...
void sigchild()
{
	...
	while (1)
	{
		switch ( (pid_child = waitpid (0, NULL, WNOHANG)))
		{
			case 0:
				return;
			case -1:
 ...
 */ 
int ExecSpawn2 (char *process_path, char *process_name)
{
	int fdfile = -1;
	time_t now;
	struct tm tmnow;
	...
	case 0:/*child*/
		...
		fdfile = open (logbuff, O_WRONLY | O_CREAT| O_APPEND, S_IRUSR | S_IWUSR);

		close (STDOUT_FILENO);
		dup (fdfile);
		close (fdfile);
		printf ("new sessid = %d", sess_id);
		execl (process_path, process_name, (void *) NULL);
		break;
	default:
		break;
	}
...
int ExecSpawn(char *process )
{
	...
	fd = open (logbuff, O_WRONLY | O_CREAT, S_IRUSR | S_IWUSR);
	...
	chldpid = spawn (pargv[0], 3, fdmap, pargv);
...
void *SocketHandler(void *lp)
{
	...
	while ( 1 )
	{
		...
		if ( state == STATE_SIGNON )
		{
			...
			{
				fprintf (stdout, "not of out client\n"  );
				free(csock);
				printf("thread_id %d returns\n", thrid);
				return 0;
			}
...
int main (int argc, char **argv)
{
	...
	pthread_t thread_id = 0;
	sprintf (appname , "%s", "XLAdmAgent" );

	/*signal initialization*/
	if (signal (SIGCHLD, sigchild) == SIG_ERR)
	{
		fprintf (stderr, "[ER] main signal");
		exit(-1);
	}
