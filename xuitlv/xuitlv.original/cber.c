...
{ 
	...
	struct cbertreenode *head; /*the beginning of child tree*/
...
typedef struct bertreeitem
{
	...
	struct bertreeitem *head; /*head of child list*/
...
/**
 * Name		: __cbertreenode_updatevalue
 ...
 *	0				: success		
 **/
static int __cbertreenode_updatevalue (PCBERTREENODE ptr, unsigned char *data, int64_t offset, int64_t length)
{
 ...
 *	0				: success		
 **/
static int __cbertreenode_updatetagvalue (PCBERTREENODE root, unsigned char *in, int maxtaglen, unsigned char *data,
	int64_t offset, int64_t length)
{
	...
	{
		if (!(root->tag[0]&0x20))
			return __cbertreenode_updatevalue (root, data, offset, length);
...
static int64_t __cbertreenode_countitemlength (PCBERTREENODE p)
{
	...
	if (p->head)
	{
		p->childlen=__cbertreenode_countitemlength(p->head);
		ret+=p->childlen;
	}
	if (p->right)
...
static int __cbertreenode_writeber (PCBERTREENODE p)
{
	...
	if (p->head)
		__cbertreenode_writeber (p->head);
...
static int __cbertreenode_serializeber (PCBERTREENODE p, unsigned char *buffer, int maxbuffer)
{
	...
	{
		i = __cbertreenode_serializeber (p->head, &buffer[ctr], maxbuffer-ctr);
		if (i < 0)
			return i;
		ctr+=i; 
	}
...
{ 
	...
	if (p->head)
	{
		__cbertreenode_drywriteber (p->head);	
	} 
	if (p->right)
...
{ 
	...
	node->comp.updatevalue = 
		(int(*)(PTREECOMPONENT, unsigned char *,int64_t, int64_t))__cbertreenode_updatevalue;
...
static int __bertreeitem_initialize(PBERTREEITEM newItem, PBERTREEITEM parent, unsigned char *tag)
{
	...
	int len = 0; 
	PBERTREEITEM child = newItem->head;
	if (tag!=NULL)
	{
