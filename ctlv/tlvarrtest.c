...
{
	fprintf (dest, "usage %s -[svri] hexbin data ....\n", appName); 
	fprintf (dest, "v verbose test , it will read hexbin data and construct tree (deserializing).\n");
	fprintf (dest, "  it will then serialize data to stream and print as hexbin data.\n");
	fprintf (dest, "s performance test , it will count how many deserialize , serialize , reset\n");
	fprintf (dest, "  tree performed in 1 second.\n");
	fprintf (dest, "r resource test, it will deserialize stream, serialize tree, reset tree \n");
	fprintf (dest, "  infinitely. you can check resource used using top(linux) or topas(aix)\n");
	fprintf (dest, "  watch the memory usage\n");
	fprintf (dest, "i interactive test, it will enable you to update and get data in an interactive\n");
	fprintf (dest, "  mode\n"); 
	return 0;
...
{ 
	...
	for (i=0; i<argc-(1+stest+vtest+stest+rtest+itest); i++)
	{
		datalist_add(datalist, newdataitem (argv[i+1+stest+
			vtest+rtest+itest]));
	}
	if (lvtest)
	{
