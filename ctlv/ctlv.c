/**
 * gcc -ggdb3 -Wall -m32 -fPIC -g -c ctlv.c
 ...
 *
 **/
typedef struct tlvtreenode
{ 
	TREECOMPONENT comp;
	struct tlvtreenode *up; /*up sibling*/
	struct tlvtreenode *left; /*left sibling*/
	struct tlvtreenode *right; /*right sibling*/
	struct tlvtreenode *parent; /*parent*/
	struct tlvtreenode *head; /*the beginning of child tree*/
...
extern PTLVTREENODE newtlvtreenode (PTLVTREENODE parent, unsigned char *tag);

/**
 * Name		: insert
 * Description	: insert to head (pto). If such item exist no
 * Input
 *	pto	: root
 *	in	: node insert
 ...
 *		  returns in.
 **/
PTLVTREENODE insert (PTLVTREENODE pto, PTLVTREENODE in)
{
	int tagCmpRes = 0;
	if (pto == NULL)
		return in;
	tagCmpRes = tagCmp (in->tag, pto->tag);
	if (!tagCmpRes)
		return pto;
	else if (tagCmpRes >0)
	{
 ...
 **/
PTLVTREENODE inserttag (PTLVTREENODE parent, PTLVTREENODE pto, unsigned char *tag)
{
	int tagCmpRes = 0;
	PTLVTREENODE ret = 0;
	if (pto == NULL)
		return newtlvtreenode (parent, tag);
	tagCmpRes = tagCmp (tag, pto->tag);
	if (!tagCmpRes)
		return pto;
	if ((tagCmpRes > 0) && pto->right) 
		return inserttag (parent, pto->right, tag); 
	else if ((tagCmpRes <0) && pto->left)
		return inserttag (parent, pto->left, tag);
	ret = newtlvtreenode (parent, tag);
	if (!ret) 
		return ret;
	if (tagCmpRes>0)
		pto->right = ret;
 ...
 *
 **/
PTLVTREENODE postorder (PTLVTREENODE p)
{
	if (!p->right)
		return p;
 ...
 **/
PTLVTREENODE preorder (PTLVTREENODE p)
{
	if (!p->left)
		return p;
 ...
 **/
PTLVTREENODE inorderpre (PTLVTREENODE p)
{
	if (!p->left)
		return NULL;
 ...
 **/
PTLVTREENODE inorderpost (PTLVTREENODE p)
{
	if (!p->right)
		return NULL;
 ...
 **/
PTLVTREENODE searchnodetag (PTLVTREENODE p, unsigned char *tag)
{
	int tagCmpRes = 0;
	if (!p)
		return NULL;
	tagCmpRes = tagCmp (tag, p->tag);
	if (!tagCmpRes)
		return p;
	else if (tagCmpRes < 0)
 ...
 **/
int siblingdelete (PTLVTREENODE p)
{
	if (!p)
		return 0;
	if (p->left)
 ...
 **/
int cleanupchild(PTLVTREENODE p)
{
	if (p)
		siblingdelete(p->head); 
	return 0;
 ...
 **/
static int __tlvtreenode_move (PTLVTREENODE dst, PTLVTREENODE src)
{
	int num = 0;
 ...
 **/
static int  __tlvtreenode_delete(PTLVTREENODE p)
{
	PTLVTREENODE cand = NULL;
	...
	{
		__tlvtreenode_move (p, cand);
		return __tlvtreenode_delete(cand);
	} 
	return 0;
}
 ...
 *	-3	: Not a valid root
 **/
static int __tlvtreenode_add(PTLVTREENODE root, unsigned char *in, int maxtaglen, int valsize)
{
	int taglen; 
	PTLVTREENODE newItem;
	unsigned char *k;
	if (!root)
	...
	if (!root->head) 
		root->head = newItem;
	return __tlvtreenode_add (newItem, k, maxtaglen - taglen, valsize); 
}
/**
 * Name		: __tlvtreenode_deletetag
 ...
 **/
static int __tlvtreenode_deletetag(PTLVTREENODE root, unsigned char *tag, int maxtaglen)
{
	int taglen1 = 0;
	int taglen2 = 0;
	unsigned char *k;
	PTLVTREENODE match = NULL;
	...
	if (!taglen2)
		return __tlvtreenode_delete(match);		
	return __tlvtreenode_deletetag (match, k, maxtaglen - taglen1);	
}
/**
 * Name		: __tlvtreenode_updatevalue
 ...
 *	0				: success		
 **/
static int __tlvtreenode_updatevalue (PTLVTREENODE ptr, unsigned char *data, int64_t offset, int64_t length)
{
...
/**
 * Name		: __tlvtreenode_updatetagvalue
 ...
 *	0				: success		
 **/
static int __tlvtreenode_updatetagvalue (PTLVTREENODE root, unsigned char *in, int maxtaglen, unsigned char *data,
	int64_t offset, int64_t length)
{
	int	taglen = 0; 
	PTLVTREENODE newItem = NULL;
	unsigned char *k;
	if (!root)
		return CTLV_NULLROOT;
	taglen = getNextTag (in, maxtaglen);
	...
	{
		if (!(root->tag[0]&0x40))
			return __tlvtreenode_updatevalue (root, data, offset, length);
	...
	if (!root->head)
		root->head = newItem;
	return __tlvtreenode_updatetagvalue(newItem, k, maxtaglen - taglen, data, offset, length); 
}
/**
 * Name		: __tlvtreenode_countitemlength
 * Description	: count all child length and save it in childlen
 * Input
 *	p	: pointer to item to count
 * Returns	: childlen + taglen + valuelen 
 **/
static int64_t __tlvtreenode_countitemlength (PTLVTREENODE p)
{
	int64_t ret = 0;
	int taglen = getNextTag (p->tag, MAX_SINGLE_TAGLEN); 
	if (p->left) 
		ret+=__tlvtreenode_countitemlength(p->left);
	if (p->head)
	{
		p->childlen=__tlvtreenode_countitemlength(p->head);
		ret+=p->childlen;
	}
	if (p->right)
		ret+=__tlvtreenode_countitemlength(p->right); 
	
	ret = ret + taglen + p->valuelen + calculateTlvLengthUsed (p->childlen+ p->valuelen);	
	//printTlvRoutine ("__tlvtreenode_countitemlength",p->tag);
	//fprintf (stdout,"childlen =%lld valuelen =%lld ret = %lld\n", p->childlen, p->valuelen , ret);
	return ret;
}
/**
 * Name		: __tlvtreenode_writetlv
 * Description	: write to stdout tree data and child data. write left in right
 * Input
 *	p	: pointer to item to write data for
 **/
static int __tlvtreenode_writetlv (PTLVTREENODE p)
{
	...
	if (p->head)
		__tlvtreenode_writetlv (p->head);

	if (p->right)
		__tlvtreenode_writetlv (p->right);
	return 0; 
}

/**
 * Name		: __tlvtreenode_serializetlv 
 ...
 **/
static int __tlvtreenode_serializetlv (PTLVTREENODE p, unsigned char *buffer, int maxbuffer)
{
	...
	int i = 0;
	int ctr = 0;
	//fprintf (stdout, "__tlvtreenode_serializetlv\n");	
	if (p->left)
		i= __tlvtreenode_serializetlv (p->left, &buffer[ctr], maxbuffer-ctr);
	if (i<0)
		return i;
	if (!p->parent)//root
		__tlvtreenode_countitemlength(p);
	...
	{
		i = __tlvtreenode_serializetlv (p->head, &buffer[ctr], maxbuffer-ctr);
		if (i < 0)
			return i;
		ctr+=i; 
	}
	i = 0;	
	if (p->right)
		i= __tlvtreenode_serializetlv (p->right, &buffer[ctr], maxbuffer-ctr);
	if (i<0)
		return i;
	ctr+=i;
	return ctr;
...
/**
 * Name		: __tlvtreenode_drywritetlv
 * Description	: do step needed to peform writting data, but do not printf
 * Input
 *	p	: pointer to item for data to write
 **/
static int __tlvtreenode_drywritetlv (PTLVTREENODE p)
{ 
	unsigned char lenTlv[32];

	if (p->left)
		__tlvtreenode_drywritetlv (p->left);
	...
	if (p->head)
	{
		__tlvtreenode_drywritetlv (p->head);	
	}
	
	if (p->right)
		__tlvtreenode_drywritetlv (p->right);
	return 0;
}

/**
 * Name		: __tlvtreenode_getvalue
 ...
 *	0				: success		
 **/
static int __tlvtreenode_getvalue (PTLVTREENODE p, unsigned char *buffer, int64_t offset, int64_t len, int64_t *copied)
{
	int64_t maxsize = 0; 
	if (p->tag[0] & 0x40)
		return CTLV_NOPRIMITIVETAG;
...
/**
 * Name		: __tlvtreenode_gettagvalue
 ...
 *	0				: success		
 **/
static int __tlvtreenode_gettagvalue (PTLVTREENODE root, unsigned char *tag, int maxtaglen,
	unsigned char *buffer, int64_t offset, int64_t len, int64_t *copied)
{
	...
	if (!child)
		return CTLV_TAGNOTFOUND;
	if ((tag[0] &0x40))
		return __tlvtreenode_gettagvalue(child, &tag[taglen], maxtaglen -taglen,
			buffer, offset, len, copied);
	return __tlvtreenode_getvalue (child, buffer, offset, len, copied);
}
/**
 * Name		: __tlvtreenode_initialize
 * Description	: reset variables
 **/
static int __tlvtreenode_initialize(PTLVTREENODE node, PTLVTREENODE parent, unsigned char *tag)
{
	cleanupchild (node);
	if (tag)
	{
...
/**
 * Name		: initializetlvtreenode
 * Description	: initialize this tree node
 *
 **/
int initializetlvtreenode (PTLVTREENODE node, PTLVTREENODE parent, unsigned char *tag)
{ 
	__tlvtreenode_initialize( node,  parent, tag);
	node->comp.add = (int(*)(PTREECOMPONENT, unsigned char *, int, int))__tlvtreenode_add;
	node->comp.del = (int(*)(PTREECOMPONENT))__tlvtreenode_delete;
	node->comp.deletetag = 
		(int(*)(PTREECOMPONENT, unsigned char *, int))__tlvtreenode_deletetag;
	node->comp.updatevalue = 
		(int(*)(PTREECOMPONENT, unsigned char *,int64_t, int64_t))__tlvtreenode_updatevalue;
	node->comp.updatetagvalue = 
		(int(*)(PTREECOMPONENT, unsigned char *, int, unsigned char *, int64_t, int64_t))__tlvtreenode_updatetagvalue;
	node->comp.countitemlength = (int64_t(*)(PTREECOMPONENT))__tlvtreenode_countitemlength;
	node->comp.writetlv = (int(*)(PTREECOMPONENT))__tlvtreenode_writetlv;
	node->comp.drywritetlv = (int(*)(PTREECOMPONENT))__tlvtreenode_drywritetlv;
	node->comp.serializetlv = (int(*)(PTREECOMPONENT, unsigned char*, int))__tlvtreenode_serializetlv;
	node->comp.getvalue =(int(*)(PTREECOMPONENT, unsigned char *, int64_t, int64_t, int64_t*)) __tlvtreenode_getvalue;
	node->comp.gettagvalue = 
		(int(*)(PTREECOMPONENT,unsigned char *, int, unsigned char *, int64_t, int64_t, int64_t*))__tlvtreenode_gettagvalue;
	node->comp.initialize = (int(*)(PTREECOMPONENT, PTREECOMPONENT, unsigned char *))__tlvtreenode_initialize;
	return 0;
}

/**
 * Name		: newtlvtreenode
 * Description	: create a new tlvtreenode
 * Input
 *	parent	: parent of this item
 *	tag	: tag of this item
 **/
PTLVTREENODE newtlvtreenode (PTLVTREENODE parent, unsigned char *tag)
{
	PTLVTREENODE newnode = NULL; 
	newnode = (PTLVTREENODE) calloc (1, sizeof (TLVTREENODE));
	if (!newnode)
		return newnode;
	getNextTag (tag, MAX_SINGLE_TAGLEN);
	initializetlvtreenode (newnode, parent, tag);
	return newnode;
}

/**
 * Name		: newroottlvtreenode
 * Description	: alloc memory for new item, set all necessary functions and variables 
 **/
PTLVTREENODE newroottlvtreenode ()
{
	PTLVTREENODE newItem = NULL;
	unsigned char tag[] = {0,0}; 
	newItem = (PTLVTREENODE) calloc (1, sizeof (TLVTREENODE));
	if (!newItem)
		return newItem; 
	initializetlvtreenode (newItem, NULL, tag);
	return newItem;	
}
...
static int __tlvtreeitem_add(PTLVTREEITEM root, unsigned char *in, int maxtaglen, int valsize)
{
	...
	ptrPrev = NULL;
	ptrCurr = root->head;
	while ( ptrCurr && ((nres = tagCmp (ptrCurr->tag, in)) <0))
	{
		ptrPrev = ptrCurr;
...
static int __tlvtreeitem_deletetag (PTLVTREEITEM root, unsigned char *tag, int maxtaglen)
{
	...
	if (!ptrCurr)
		return CTLV_TAGNOTFOUND;
	while (( (nres = tagCmp (ptrCurr->tag, tag)) <0)
		&& ptrCurr!=NULL)
	{
		ptrPrev = ptrCurr;
		ptrCurr = ptrCurr->next;
	...
	unsigned char *buffer, int64_t offset, int64_t len, int64_t *copied)
{
	...
	if (len < 0)
		return CTLV_INVALIDGETSETLENGTH;
	child = root->head;
	while( child && ((res = tagCmp (child->tag, tag)) <0)) 
	{
		child = child->next;
	} 
	taglen = getNextTag(tag, maxtaglen);
...
{
	PTLVTREENODE root = newroottlvtreenode ();
	PCXTLV p = (PCXTLV) calloc (1, sizeof (CXTLV));
	if (!p)
	{
		free (root);
