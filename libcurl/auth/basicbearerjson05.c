...
int getTokenFromRemote(CURL *curl, const char *url, char *username, char *password, int numTries) {
	...
	if (res != CURLE_OK) {
		...
		printf("status: %ld\n", resCode);
		printf("%s\n", chunk.memory);
		if (callGWIsSuccess(curl, chunk.memory)) {
...
struct curl_slist * createGenericAppointmentHeaders(CURL *curl, char *tokenBuff) {
	struct curl_slist *headers = NULL;
	char authStr [1024] = {0};
...
int createAppointmentInCallGW(CURL *curl, char *orderId, char *foaUser, char *fbbId, char *foaPhoneNumber, char ** customerPhones, int customerPhoneLength, char *appointmentDate, char* appointmentTime) {
	...
	if (tokenNotExists && getTokenFromUrl(curl, getTokenUrl)) {
		...
		struct curl_slist *headers = createGenericAppointmentHeaders(curl, tokenBuff);
		struct WriteThis wt = {
			.readptr = requestBody,
...
int main(void) {
	...
	if (curl) {
		...
		updateAppointmentInCallGW(curl, "foa-order-0018", "foa-user-0003",
			"fbb-0003", "0101234567", emptyArray, 0, "2020-11-12", "12:30:00");
		deleteAppointmentInCallGW(curl, "foa-order-0018");
