...
int createAppointmentInCallGW(CURL *curl, char *orderId, char *foaUser, char *fbbId, char *foaPhoneNumber, char ** customerPhones, int customerPhoneLength, char *appointmentDate, char* appointmentTime) {
	...
	fprintf(stdout, "getTokenUrl = %s\ntokenNotExists = %d\n", getTokenUrl, tokenNotExists);
	if (tokenNotExists && getTokenFromUrl(curl, getTokenUrl)) {
		...
		//CURLOPT_POSTFIELDS implies method is POST
		curl_easy_setopt(curl, CURLOPT_POSTFIELDS, requestBody);
		curl_easy_setopt(curl, CURLOPT_POSTFIELDSIZE_LARGE, (curl_off_t)strlen(requestBody));
		...
		if (res != CURLE_OK) {
			...
			json_object_to_json_string_ext(resultRoot, JSON_C_TO_STRING_PRETTY);
