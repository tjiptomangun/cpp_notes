...
int main(int argc, char **argv) {
	...
	const char *sOIDName = "1.3.6.1.2.1.1.4.0";
	//const char *sOIDName = "system.sysContact.0";
	//if (!snmp_parse_oid(sOIDName, anOID, &anOID_len)) {
	if (!read_objid(sOIDName, anOID, &anOID_len)) {
		snmp_perror(sOIDName);
		snmp_log(LOG_ERR, sOIDName);
		exit(3);
